...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ОсновныеНачисления.Количество() = 0 
		И ДополнительныеНачисления.Количество() = 0 Тогда
...
Процедура СформироватьДвиженияПоОсновнымНачислениям()
	...
	Пока Выборка.Следующий() Цикл
		...
		// Пропустить записи с нулевым или несуществующим окладом
...
Процедура РассчитатьНДФЛ()
	...
	// Получаем базу НДФЛ напрямую из регистров расчёта.
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	БазовыеНачисления.Сотрудник КАК Сотрудник,
		|	СУММА(БазовыеНачисления.Результат) КАК БазаНДФЛ
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|	ГДЕ
		|		ВКМ_ДополнительныеНачисления.Регистратор = &Ссылка) КАК БазовыеНачисления
		|СГРУППИРОВАТЬ ПО
		|	БазовыеНачисления.Сотрудник";
	
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	// Соответствие «Сотрудник → База НДФЛ»
	БазаПоСотрудникам = Новый Соответствие;
	
	Пока Выборка.Следующий() Цикл
		БазаПоСотрудникам.Вставить(Выборка.Сотрудник, Выборка.БазаНДФЛ);
	КонецЦикла;
	
	// Заполняем результат в уже созданных движениях НДФЛ
	Для Каждого Движение Из Движения.ВКМ_Удержания Цикл
		...
		База = БазаПоСотрудникам[Движение.Сотрудник];
		Если База = Неопределено Тогда
			База = 0;
		КонецЕсли;
		
		Движение.Результат = База * 13 / 100;
...
Процедура СформироватьДвиженияВыполненныеСотрудникомРаботы(Отказ)
	...
	// Блокировка ДО формирования движений
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СуммаКОплате > Выборка.СуммаКОплатеОстаток Тогда
			...
			ТекстСообщения = СтрШаблон(
				"Документ не проведен. Для сотрудника %1 сумма начисления (%2) больше чем сумма за выполненные работы (%3)",
