...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание Тогда 
		...
		ИндексОснования = ПроверяемыеРеквизиты.Найти("Основание");
		Если ИндексОснования <> Неопределено Тогда
    		ПроверяемыеРеквизиты.Удалить(ИндексОснования);
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,"ВидДоговора") = Перечисления.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание Тогда
		...
		ИначеЕсли  ВКМ_ДоработкаДокументаРеализацияТоваровИУслугВызовСервера.НайтиСуществующийДокументРеализацияСДаннымиТекущегоОбъекта(ЭтотОбъект) <> Неопределено Тогда
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Иначе 
		...
		СоздатьДвиженияBKM_ВыполненныеРаботыКлиенту(Отказ);
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
...
Процедура СоздатьДвиженияBKM_ВыполненныеРаботыКлиенту(Отказ)
	
	// Дополнительная проверка договора
	Если Не  ВКМ_ДоработкаДокументаРеализацияТоваровИУслугВызовСервера.ДокументРеализацииТоваровИУслугЗаполненВерно(ЭтотОбъект) Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура BKM_ВыполнитьАвтозаполнение() Экспорт
	...
	|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОстаткиИОбороты.Договор.ВКМ_СтоимостьЧасаРаботыСпециалиста, 0) КАК ДоговорВКМ_СтоимостьЧасаРаботыСпециалиста
	...
	|	1 КАК Количество,
	|	ДоговорыКонтрагентов.ВКМ_ЕжемесячнаяАбонентскаяПлата КАК Цена,
	|	ДоговорыКонтрагентов.ВКМ_ЕжемесячнаяАбонентскаяПлата КАК Сумма
	|ИЗ
	|	Константа.ВКМ_НоменклатураАбонентскаяПлата КАК ВКМ_НоменклатураАбонентскаяПлата,
	|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|ГДЕ
	|	ДоговорыКонтрагентов.Ссылка = &Договор
	|	И ДоговорыКонтрагентов.ВКМ_ЕжемесячнаяАбонентскаяПлата > 0
